<template>
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="flex items-center space-x-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-800">Edit Subset</h1>
                <p class="text-sm text-slate-500 mt-1">Update the details of this subset.</p>
            </div>
        </div>
        ...
        <div v-if="initialLoading" class="bg-white rounded-2xl shadow-sm border border-slate-200 p-12 flex flex-col items-center justify-center min-h-[300px]">
            ...
            <p class="text-slate-500 font-medium">Loading subset details...</p>
        </div>
        ...
        <div v-else class="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
            <form @submit.prevent="submitForm" class="space-y-6">
                ...
                <div class="pt-4 flex items-center justify-end space-x-3 border-t border-slate-100">
                    <router-link to="/user/subsets" class="px-5 py-2.5 text-sm font-semibold text-slate-600 hover:text-slate-800 transition-colors" :class="{'pointer-events-none opacity-50': submitting}">
                        Cancel
                    </router-link>
                    
                    <button
                        type="submit"
                        :disabled="submitting || !form.subset_name.trim()"
...
const subsetId = route.params.id;
...
onMounted(async () => {
    try {
        const response = await axios.get(`/api/v1/subsets/${subsetId}`);
        const subsetData = response.data.subset;
        
        form.subset_name = subsetData.subset_name;
        form.subset_descr = subsetData.subset_descr || '';
    } catch (error) {
        console.error("Failed to fetch subset:", error);
        alert('Subset not found or you do not have permission.');
...
const submitForm = async () => {
    ...
    try {
        await axios.put(`/api/v1/subsets/${subsetId}`, form);
        router.push('/user/subsets');
    } catch (error) {
        console.error("Failed to update subset:", error);
        if (error.response?.data?.errors) {
            errors.value = Object.values(error.response.data.errors).flat();
        } else {
            errors.value = [error.response?.data?.message || 'Failed to update subset. Please try again.'];
        }
    } finally {
